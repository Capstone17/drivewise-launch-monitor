...
def process_video(
    ...
    lost_thresh = 3
    mode = "search"
    yolo_usage = int(max(0, min(100, yolo_usage)))
    if yolo_usage >= 100:
        verify_interval = 0
    elif yolo_usage <= 5:
        verify_interval = float("inf")
    else:
        verify_interval = max(1, int(45 * (100 / yolo_usage - 1)))
    confirm_count = 0
    lost_count = 0
...
if __name__ == "__main__":
    ...
    yolo_usage = int(sys.argv[6]) if len(sys.argv) > 6 else 50
    process_video(
        video_path,
        ball_path,
        sticker_path,
